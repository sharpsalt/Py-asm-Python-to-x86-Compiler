...
static int genAST(struct ASTNode *n){
    ...
    if (n->op == A_ASSIGN) {
        rightregister = genAST(n->right);  // Eval expr first
        cgstorglob(n->left->ident, rightregister);  // Store to var
        return rightregister;  // Return value (no free, caller handles)
    }
    ...
    case A_FLOAT:return (cgload((int)n->floatvalue));
    case A_IDENT: return cgloadglob(n->ident);

        // NEW: A_ASSIGN - store right to left's global, return right's reg
        case A_ASSIGN: {
            int targetreg = cgloadglob(n->left->ident);  // Load addr? Wait, no—use cgstorglob
            cgstorglob(n->left->ident, rightregister);
            free_register(rightregister);
            return targetreg;  // Or return rightreg? Semantics: return value
        }
    case A_GLUE:
